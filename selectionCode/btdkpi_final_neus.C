...
void btdkpi_final::Loop(TString KorPi, Int_t cut_code, Long64_t nentries)
{
  ...
  Int_t style(1);
  if (_sig_type=="btdsk"||_sig_type=="btdk"||_sig_type=="btdspi"||_sig_type=="btdpi") style=2;
  if (_sig_type=="chb") style=3;
  if (_sig_type=="chbk"||_sig_type=="chbpi") style=4;
  if (_sig_type=="b0b0bar") style=5;
  if (_sig_type=="bbbar") style=3;
  if (_sig_type=="bbbark"||_sig_type=="bbbarpi") style=4;
...
void btdkpi_final::LoopBest(TString KorPi, Int_t cut_code, Long64_t nentries, Int_t opt ,Int_t particular_cut)
{
  ...
  if (fChain == 0) return;
  assert(((cut_code<=115 ||cut_code==1000) && particular_cut==-999)||(cut_code>500 && cut_code<520 && particular_cut>0 && particular_cut<20));
  ...
  */  
  ...
  vect[9]=Tdeltadeltamd;
  TH1F* TmassD0 = new TH1F("massD0","massD0 distribution",nbins,1.840,1.890);
  vect[8]=TmassD0;
 ...
 //
  ...
  Int_t ddecay_code = (_dtype=="kspipi")? 1 : 2 ;   // 1=kspipi, 2=kskk
  ...
  for (Int_t jentry=0; jentry<nentries;jentry++) 

    {
      ...
      if(Cut(KorPi,cut_code,kFALSE,particular_cut)<0) continue;
  ...
  if (cut_code==100 || cut_code==2) {
    ...
    canvas_3->Print(filename_ASCII+"_demk_BkgTreeMCmatches.eps");
    canvas_3->Print(filename_ASCII+"_demk_BkgTreeMCmatches.gif");
  }
...
void btdkpi_final::Fits(TString KorPi, Int_t cut_code)
{
  TString sdec=  "btdkpi_";
  char cut_code_str[10];
  sprintf(cut_code_str,"Cut%i",cut_code);
  ...
  while ((var = (RooRealVar*)iter->Next())) 
	...
	continue;
      ...
      if (pdf) {
	Double_t lo = (name=="demk")? -0.100 : var->getMin();
	Double_t hi = (name=="demk")?  0.120 : var->getMax();
...
RooAbsPdf* btdkpi_final::getPdf(TString KorPi, RooRealVar& var) 
{
  ...
  if (varname=="mes") {
    ...
    RooRealVar* mesGWidth = new RooRealVar("mesGWidth","mES Gaussian width",0.0025,"GeV") ;
    ...
    if (_sig_type=="btdsk" || _sig_type=="btdk" || _sig_type=="btdspi" || _sig_type=="btdpi" || 
	_sig_type=="chb" || _sig_type=="chbk" || _sig_type=="chbpi" || 
	_sig_type=="b0b0bar" || _sig_type=="bbbar" || _sig_type=="bbbark" || _sig_type=="bbbarpi" || _sig_type=="on" ) {
      //if (_sig_type=="chb" || _sig_type=="b0b0bar") {
      //  mesGWidth->setVal(0.0038); mesGWidth->setConstant(); 
      //}
      return mesPdf;
      //mesGMean->setConstant();
      //return mesPdfBG;
    } else if (_sig_type=="uds" || _sig_type=="ccbar" || _sig_type=="udsccbar" || _sig_type=="off") {
      return mesArgPdf;
    }  else assert(0);   
  }//end mES if 
  ...
  if (varname=="demk") {
    ...
    RooRealVar* fdeG = new RooRealVar("fdeG","DeltaE Gaussian fraction",0.7,0,1) ;  
    //RooRealVar* fdeG2 = new RooRealVar("fdeG2","DeltaE Gaussian fraction 2",0.15,0,1) ;  
    RooRealVar* dePolyC1 = new RooRealVar("dePolyC1","DeltaE polynomial coefficient C1",0.,-1e4,1e4) ;
    RooRealVar* dePolyC2 = new RooRealVar("dePolyC2","DeltaE polynomial coefficient C2",0.,-1e5,1e5) ; 
    ...
    RooAbsPdf* dePdf = new RooAddPdf("dePdf","DeltaE PDF",RooArgList(*deGPdf,*deExpPdf,*dePolyPdf),RooArgList(*fdeG,*fdeExp)) ;
    //RooAbsPdf* deSigPdf = new RooAddPdf("deSigPdf","DeltaE PDF signal",RooArgList(*deGPdf,*deG2Pdf,*dePolyPdf),RooArgList(*fdeG,*fdeG2)) ; 
    RooAbsPdf* deSigPdf = new RooAddPdf("deSigPdf","DeltaE PDF signal",RooArgList(*deGPdf,*dePolyPdf),*fdeG) ;
    if(KorPi=="pi") deGMean->setVal(0.050);
    if (_sig_type=="btdsk" || _sig_type=="btdk" || _sig_type=="btdspi" || _sig_type=="btdpi") {
      //dePolyC1->setConstant();
      return deSigPdf;
    }
    if (_sig_type=="chb" || _sig_type=="bbbar" || 
	_sig_type=="chbk" || _sig_type=="chbpi" || 
	_sig_type=="bbbark" || _sig_type=="bbbarpi" ||_sig_type=="b0b0bar" ||
	_sig_type=="on") {
    ...
    if (_sig_type=="uds" || _sig_type=="ccbar" || _sig_type=="udsccbar" || _sig_type=="off")
      return dePolyPdf;
    assert(0);  
  }//end demk if
  ...
  if (varname=="fisherDK") {
    ...
    RooRealVar* fiG2Mean    = new RooRealVar("fiG2Mean","Fisher 2nd Gaussian mean",0.84,-6,6) ;
    ...
    RooRealVar* fiG1Frac    = new RooRealVar("fiG1Frac","Fisher fraction of 1st Gaussian",0.7,0,1) ;
    RooAbsPdf* fiPdf(0);
    if (_sig_type=="btdsk" || _sig_type=="btdk" || _sig_type=="btdspi" || _sig_type=="btdpi" || _sig_type=="chbk" || _sig_type=="chbpi" || 
	_sig_type=="bbbark" || _sig_type=="bbbarpi" || _sig_type=="on" || _sig_type=="chb" || _sig_type=="b0b0bar" || _sig_type=="bbbar") {
...
{
  if (_sig_type!="chb" && _sig_type!="b0b0bar") return 0;
  //
