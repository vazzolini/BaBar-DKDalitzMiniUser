...
void btdkpi::Loop(Int_t nentries)
{
  ...
  for(Int_t Dentry=0;Dentry<=1;Dentry++){
	...
	if (_super_sig_type=="chb" || _super_sig_type=="chbk" || _super_sig_type=="chbpi" || 
...
Int_t btdkpi::isSignalEvt(Int_t lundD0Trk,Int_t Dlund,Int_t i_chb){
  ...
  if (_super_sig_type=="chb" || _super_sig_type=="chbk" || _super_sig_type=="chbpi" || 
	...
	if (truth) {
	  ...
	  // 	    cout<<"Dstar0dau..."<<endl;
	  // 	    printInfo(Dstar0dau);
	  // 	    cout<<"fine Dstar0dau..."<<endl;
	  // 	    cout<<"D0dau..."<<endl;
	  // 	    printInfo(D0dau);
	  // 	    cout<<"fine D0dau..."<<endl;
	  // 	    cout<<"bdau[2] = prdau"<<bdau[2]<<endl;
	  // 	    cout<<"D0k0dau..."<<endl;
	  // 	    printInfo(D0k0dau);
	  // 	    cout<<"fine D0k0dau..."<<endl;
	  // 	    cout<<"D0Ksdau..."<<endl;
	  // 	    printInfo(D0Ksdau);
	  // 	    cout<<"fine D0Ksdau..."<<endl;
    ...
    // match prompt track    
    if (indexMcpr==bdau[2] || indexMcpr==bdau[1]) out+=4; 

    // match Pi0 from D*0
    if (indexMcPi0g1== Pi0dau[1] || indexMcPi0g1==Pi0dau[2]) out+=8; //Pi0g1 
    if ((indexMcPi0g2== Pi0dau[2] || indexMcPi0g2==Pi0dau[1]) && (Pi0dau[2]!=Pi0dau[1])) out+=16; //Pi0g2 

    // match Gamma from D*0
    if (indexMcGam==Dstar0dau[2] || indexMcGam==Dstar0dau[1]) out+=32; //Gam 
