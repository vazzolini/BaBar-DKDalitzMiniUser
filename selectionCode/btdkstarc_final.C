...
void btdkstarc_final::Loop(Int_t cut_code, Long64_t nentries)
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     Float_t fisherDK=0.93416-0.939082*fabs(costhrust)+0.0322243*legendrep0-0.207507*legendrep2-0.272079*fabs(cosChBCM);
...
void btdkstarc_final::LoopBest(Int_t cut_code, Long64_t nentries, Int_t opt)
{
  ...
  for (Int_t jentry=0; jentry<nentries;jentry++) {
	...
	//********************************************************************
	//	if(actualisMcD0){  
	//if((abs(actualisMcD0id)==421 && ((_isCS==kFALSE && abs(actuallundVectorc)==323)||(_isCS==kTRUE && abs(actuallundVectorc)==20213)))){
	//  if((actualMcEvtType&3869==3869 || actualMcEvtType&3870==3870))totSignalRSbc++;
	//  else if((actualMcEvtType&3841==3841 || actualMcEvtType&3842==3842) 
	//    && (actualMcEvtType&29!=29 && actualMcEvtType&30!=30) 
	//    ){
	//    totSignalWSbc++;
