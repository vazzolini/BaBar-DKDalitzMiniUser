#ifndef btdkpi_final_ebe_h
#define btdkpi_final_ebe_h
...
class btdkpi_final_ebe {
  ...
  //btdkpi_final_ebe(TString dtype, TString dstar0mode, TString d0mode, TString sig_type, TString release="analysis-30", Bool_t doReadRootuples=kTRUE);
   //  btdkpi_final_ebe(TString dtype, TString dstar0mode, TString d0mode, TString sig_type, TString release="analysis-32", Bool_t doReadRootuples=kTRUE);
   //  btdkpi_final_ebe(TString dtype, TString dstar0mode, TString d0mode, TString sig_type, TString release="rel-24", Bool_t doReadRootuples=kTRUE);
  btdkpi_final_ebe(TString dtype, TString dstar0mode, TString d0mode, TString sig_type, TString release="rel-18new", Bool_t doReadRootuples=kTRUE);
  virtual ~btdkpi_final_ebe();
...
#ifdef btdkpi_final_ebe_cxx

/*
  btdkpi_final_ebe::btdkpi_final_ebe(TTree *tree) {
...
btdkpi_final_ebe::btdkpi_final_ebe(TString dtype, TString dstar0mode, TString d0mode, TString sig_type, TString release, Bool_t doReadRootuples)
{
  ...
  if (release=="analysis-30") filename_root="./prod-31-skim18b/DK/reduced/";
  if (release=="analysis-32") filename_root="./prod-32-skim18c/DK/reduced/";
  if (release=="rel-24")      filename_root="./validation-rel24/DK/reduced/";
  if (release=="rel-18new")   filename_root="./vali1/";
  //   if (release=="rel-18new")   filename_root="./vali1/Dstar/";
  //  if (release=="rel-24") filename_root="./reduced/";

   //if (release=="analysis-32") filename_root="./theroot/";
  ...
  assert(_sig_type == "btdk"      || _sig_type == "btdsk"      ||
	 _sig_type == "btdk1234"  || _sig_type == "btdk5"      ||
	 _sig_type == "btdk1"     || _sig_type == "btdsk1"     ||
	 _sig_type == "btdsk1234" || _sig_type == "btdsk5"     ||
  ...
  if (doReadRootuples) {
    ...
    if (_sig_type=="on") {
      ...
      //cout << "filename : " << filename << endl;
      //tree->Add(filename);
      //filename=filename_root+"on7.root";
      //cout << "filename : " << filename << endl;
      //tree->Add(filename);
      ...
      cout << "filename : " << filename << endl;
      tree->Add(filename);
      filename=filename_root+"chbk5.root";
      cout << "filename : " << filename << endl;
      tree->Add(filename);
      ...
      cout << "filename : " << filename << endl;
      tree->Add(filename);
      filename=filename_root+"chbpi5.root";
      cout << "filename : " << filename << endl;
      tree->Add(filename);
...
btdkpi_final_ebe::~btdkpi_final_ebe()
{
   if (!fChain) return;
...
Int_t btdkpi_final_ebe::GetEntry(Long64_t entry)
{
...
Long64_t btdkpi_final_ebe::LoadTree(Long64_t entry)
{
...
void btdkpi_final_ebe::Init(TTree *tree)
{
...
Bool_t btdkpi_final_ebe::Notify()
{
...
void btdkpi_final_ebe::Show(Long64_t entry)
{
...
Int_t btdkpi_final_ebe::Cut(TString KorPi, Int_t cut_code, Bool_t mctruth)
{  
  // select channel
  ...
  if( 
     ...
     ((cut_code==113 && _d0mode=="kspipi")? ((massD0>(mD0_nom-0.040) && massD0<(mD0_nom-0.020)) || (massD0>(mD0_nom+0.020) && massD0<(mD0_nom+0.040))): kTRUE)
      ...
      //(_d0mode=="kspipi"? kTRUE :  (int(kaonidD0Pi1)&pid_cut_k_kskk_1k)==pid_cut_k_kskk_1k || (int(kaonidD0Pi2)&pid_cut_k_kskk_1k)==pid_cut_k_kskk_1k)
      &&
      ((cut_code==2 || cut_code==112 || cut_code==113 || cut_code==115)? (demk2>=demk_lo && demk2<=demk_hi) : (demk2>=demk_lo_cut && demk2<=demk_hi_cut)) &&
      (cut_code==8? kTRUE : (chi2D0>=0. && probChi2D0>probChi2D0_cut)) &&
      ((cut_code==1 || cut_code==112 || cut_code==113 || cut_code==114 || cut_code==115)? (mes>=mes_lo && mes<=mes_hi) : mes>mes_cut) && 
...
Double_t btdkpi_final_ebe::Rescaling(TLorentzVector& vks, TLorentzVector& vpi1, TLorentzVector& vpi2, Double_t M, Double_t mass1, Double_t mass2, Double_t mass3) { 
  ...
  while (fabs(c_lo-c_hi)>=eps) {   
    ...
    //if(c_lo>0.) cout<<"PANIC in btdkpi_final_ebe::Rescaling: c_lo>0. = "<<c_lo<<endl; 
    //assert(c_lo<=0.); 
    alpha = hi; 
    E1p=sqrt(m1*m1+alpha*alpha*p1*p1); 
    E2p=sqrt(m2*m2+alpha*alpha*p2*p2); 
    ...
    //if(c_hi<0.) cout<<"PANIC in btdkpi_final_ebe::Rescaling: c_hi<0. = "<<c_hi<<endl; 
    //assert(c_hi>=0.); 
...
#endif // #ifdef btdkpi_final_ebe_cxx 
