#define btdkpi_final_ebe_cxx
#include "btdkpi_final_ebe.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iomanip.h>
...
void btdkpi_final_ebe::Loop(TString KorPi, Int_t cut_code, Long64_t nentries)
{
  if (fChain==0) return;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
	...
	break;
      ...
      fisherDK=0.93416-0.939082*fabs(costhrust)+0.0322243*legendrep0-0.207507*legendrep2-0.272079*fabs(cosChBCM); 
...
Double_t btdkpi_final_ebe::getchi(Float_t mPi0,Float_t mDstar0, Float_t mD0)
{
...
void btdkpi_final_ebe::LoopBest(TString KorPi, Int_t cut_code, Long64_t nentries, Int_t opt)
{
  ...
  //
  gROOT->SetStyle("Plain");//desactiva el color by default de root
  if (fChain == 0) return;
 ...
 //
  ...
  //eventbyevent:
  TString newrootfile_name;

  newrootfile_name="./vali1/ASCII/final_valireduced_"+_dstar0mode+KorPi+"_"+_d0mode+"_"+_sig_type+"_Bbest_"+cut_code_str+"_"+_release+".root";  
  TFile *filevali = new TFile(newrootfile_name,"RECREATE","Reduced root file");
  TTree *treevali = new TTree("treevali","Reduced ROOT tree"); 
  treevali->Branch("actualmes",&actualmes,"actualmes/D");
  treevali->Branch("actualdemk",&actualdemk,"actualdemk/D");
  treevali->Branch("actuallowerID", &actuallowerID, "actuallowerID/I");
  treevali->Branch("actualupperID", &actualupperID, "actualupperID/I");
  treevali->Branch("actualrunNumber",&actualrunNumber,"actualrunNumber/I");
...
void btdkpi_final_ebe::Fits(TString KorPi, Int_t cut_code, TString varname)
{
  TString sdec=  "btdkpi_";
  char cut_code_str[10];
  sprintf(cut_code_str,"Cut%i",cut_code);
  ...
  while ((var = (RooRealVar*)iter->Next())) 
	...
	xframe->getAttLine()->SetLineColor(style);//color dels punts
	...
	pdf->plotOn(xframe);
	xframe->getAttLine()->SetLineColor(kBlue);
	pdf->paramOn(xframe,Layout(0.15,0.61,0.8));
...
RooAbsPdf* btdkpi_final_ebe::getPdf(TString KorPi, RooRealVar& var) 
{
...
Int_t btdkpi_final_ebe::bkg_tree_MC(TString KorPi, 
...
{
  if (_super_sig_type!="chb" && _super_sig_type!="b0b0bar") return 0;
  //
...
void btdkpi_final_ebe::giveMeDauVisible(Int_t indexMother, Int_t* chargedDau, Int_t* neutralDau, Int_t* chargedDauSoft, Int_t* neutralDauSoft) 
{
...
void btdkpi_final_ebe::writeDecay(Int_t indexMother, ofstream& outfile)
{
  Int_t* dau = new Int_t[30];
  for(Int_t k=0; k<30;k++)dau[k]=-9;
  giveMeDau(indexMother,dau);
...
Bool_t btdkpi_final_ebe::indexExists(Int_t index, Int_t* listOfIndex)
{
  Bool_t out(false);
...
Bool_t btdkpi_final_ebe::isMcMother(Int_t indexMcMother, Int_t indexMcDau) 
{
...
Bool_t btdkpi_final_ebe::isIdMcMother(Int_t idMcMother, Int_t indexMcDau) 
{
...
Bool_t btdkpi_final_ebe::isIdMcMother(Int_t idMcMother1, Int_t idMcMother2, Int_t indexMcDau) 
{
  if (indexMcDau<0) return false;
  Int_t idx = mothMc[indexMcDau]-1;
  if (idx<0) return false;
...
Int_t btdkpi_final_ebe::bkg_tree_number(Int_t back, Int_t index, ofstream & outfile4, Int_t ientry){
...
//void btdkpi_final_ebe::bkg_tree(TString KorPi, ofstream& outfile2,Int_t ientry){
void btdkpi_final_ebe::bkg_tree(TString KorPi, ofstream& outfile2,Int_t ientry,Int_t index_B[]) {  
...
void btdkpi_final_ebe::giveMeDau(Int_t index,Int_t* dauarray){
...
void btdkpi_final_ebe::giveMeDau(Int_t* motharray, Int_t id, Int_t* dauarray){
  Int_t j(1);
...
void btdkpi_final_ebe::readPDT(const std::string fname){
  ...
  if (!indec) {
    cout << "Error in btdkpi_final_ebe::readPDT: Could not open:"<<fname.c_str()<<endl;
    return;
  }
...
const char* btdkpi_final_ebe::getName(Int_t stdhepid)
{
  TString pname = _map_pname[stdhepid];
  if (pname=="") {
    cout << "PANIC in btdkpi_final_ebe::getName: stdhepid = " << stdhepid << " unknown" << endl;
    assert(0);
  }
  return pname;
}
...
Int_t btdkpi_final_ebe::getStdhepid(const char* pname)
{
  Int_t stdhepid = _map_stdhepid[pname];
  if (stdhepid==0) {
    cout << "PANIC in btdkpi_final_ebe::getStdhepid: pname = " << pname << " unknown" << endl;
    assert(0);
  }
  return stdhepid;
}
...
Int_t btdkpi_final_ebe::getChg3(Int_t stdhepid)
{
  TString pname = _map_pname[stdhepid];
  if (pname=="") {
    cout << "PANIC in btdkpi_final_ebe::getchg3: stdhepid = " << stdhepid << " unknown" << endl;
    assert(0);
  }
...
double btdkpi_final_ebe::getCtau(Int_t stdhepid)
{
  TString pname = _map_pname[stdhepid];
  if (pname=="") {
    cout << "PANIC in btdkpi_final_ebe::getCtau: stdhepid = " << stdhepid << " unknown" << endl;
    assert(0);
  }
...
Float_t btdkpi_final_ebe::CosHely(const TLorentzVector & P,const TLorentzVector & Q, const TLorentzVector & D)
{
...
void btdkpi_final_ebe::dalVarsTruth(Int_t mcTruth,Float_t* dalVars)
{
  if(!mcTruth) return;
